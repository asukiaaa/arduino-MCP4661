...
MCP4661_asukiaaa::Core potentio(&Wire,
                                MCP4661_asukiaaa::DeviceAddress::A0L_A1L_A2L);
...
void loop() {
  ...
  uint16_t valRead;
  result = potentio.readWiper(0, &valRead);
  printLogAboutRead(&Serial, 0, valRead, result);
  result = potentio.readWiper(1, &valRead);
